...
test('parses simple docker run command', async () => {
  const cmd =
    'docker run -v /var/dir1:/var/nginx/dir1 -v /var/dir2:/var/nginx/dir2 nginx:latest';
...
test('parses simple docker run command with ignored options', async () => {
  const cmd =
    'docker run --rm -v /var/dir1:/var/nginx/dir1 -v /var/dir2:/var/nginx/dir2 nginx:latest';
  const result = composerize(cmd);
  ...
  const message = result.messages.find(
    (msg) =>
      msg.type === MessageType.notTranslatable && msg.value.includes('--rm')
  );
...
test('parses simple docker run command with custom command', async () => {
  const cmd =
    'docker run -v /var/dir1:/var/nginx/dir1 -v /var/dir2:/var/nginx/dir2 nginx:latest nginx -t';
  const result = composerize(cmd);
  const expected = YAML.load(
    path.join(__dirname, './data/custom-command.yaml')
  );
  expect(result.yaml).toEqual(YAML.stringify(expected, 9, 4));
});
